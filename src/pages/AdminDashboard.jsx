...
export default function AdminDashboard() {
  ...
  const [dataInicioFiltro, setDataInicioFiltro] = useState(
    formatarDataInput(seteDiasAtras)
  );
  const [dataFimFiltro, setDataFimFiltro] = useState(formatarDataInput(hoje));
    ...
    try {
      const [faturamento, ticket, vendasIntervalo, top5Cursos] =
        await Promise.all([
          ...
          dashboardService.buscarVendasPorIntervalo(
            dataInicioFiltro,
            dataFimFiltro
          ),
  ...
  async function filtrarVendasPorPeriodo() {
    try {
      ...
      const vendasIntervalo = await dashboardService.buscarVendasPorIntervalo(
        dataInicioFiltro,
        dataFimFiltro
      );
...
const datasVendas = vendasPorDia.map((item) =>
  item.dataCompra.split("-").reverse().slice(0, 2).join("/")
);
  ...
  const kpis = [
    ...
    },
    {
      ...
      valor: `${variacaoPositiva ? "+" : ""}${variacaoLucroPercentual.toFixed(
        1
      )}%`,
      icon: <TrendingUp size={24} />,
                ...
                <div className="flex flex-col gap-6 mb-8">
                  ...
                  <div className="flex flex-col md:flex-row gap-3">
                    <input
                      ...
                      className="border border-[#ece7e2] rounded-2xl px-4 py-2 text-sm text-gray-600 outline-none focus:border-[#c9a46c]"
                    />
                    ...
                    <button
                      type="button"
                      onClick={filtrarVendasPorPeriodo}
                      className="bg-[#c9a46c] text-white rounded-2xl px-5 py-2 text-sm hover:bg-[#b89255] transition-all"
                    >
                      Filtrar
                    </button>
                  </div>
